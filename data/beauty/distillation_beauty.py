...
filtering = ['The user prefers X, and the item\'s attributes are X:',
                'User preference: ',
                'User\'s Preference: ',
                'User Preference: ',
                'Item attributes: ',
...
def main(
    ...
    f = open('./data/beauty/explanation.json', 'r')
    data = json.load(f)
    f.close()
    ...
    # 保存回JSON文件
    with open('./data/beauty/explanation_rationale.json', 'w') as file:
